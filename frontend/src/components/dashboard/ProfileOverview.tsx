...
export function ProfileOverview({ name, handle, stats }: ProfileOverviewProps = {}) {
  const { user } = useAuth();

  const displayName = name || user?.username;
  if (!displayName) return null;

  const displayHandle = handle || (user ? `@${user.username}` : "");
  ...
  const resolvedStats = stats ?? { likes: 0, saved: 0, subscribers: 0, uploads: 0 };
