...
export default function FeedPage() {
  ...
  const handleVideoSelect = (video: Video) => {
    ...
    setInteractionError('')
    // BUG FIX: restore the user's actual like/dislike state from the server-provided video data
    // instead of always resetting to false, which caused the buttons to show wrong state after
    // the user had already liked/disliked a video and then switched away and back.
  ...
  const handleToggleWatchLater = async () => {
    ...
    try {
      ...
      // Backend returns { watchingLater: boolean, videoId } — re-fetch the full list
      if (res.data?.watchingLater !== undefined) {
        fetchWatchLater()
      } else {
                    ...
                    <div className="flex h-full min-h-[620px] flex-col rounded-3xl border border-border bg-[#111] p-4">
                      ...
                      <div className="mt-4 min-h-0 flex-1 overflow-hidden rounded-2xl border border-border bg-[#111]">
                        <CodePane
                          ...
                          testCaseCount={activeVideo.codePane.testCases?.length ?? 0}
                        />
                      </div>
                    </div>
                  </div>
