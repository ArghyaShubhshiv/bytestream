...
import { getCachedVideoFeed, clearVideoFeedCache } from '../hooks/usePreloadVideos'
...
export default function FeedPage() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => setAppliedQuery(search), 350)
    return () => clearTimeout(timer)   // cancel if you type again before 350ms
  }, [search])
  ...
  useEffect(() => {
          ...
          if (cached && Array.isArray(cached)) {
            ...
            try {
              ...
              if (feedVideos.length > 0) {
                ...
                // Re-sync the active video to the freshly-annotated copy so its
                // like state and counts reflect the server, not the stale cache.
                setActiveVideo((current) => {
                  if (!current) return feedVideos[0]
                  const fresh = feedVideos.find((video) => video.id === current.id)
    ...
    loadFeed(appliedQuery)
  }, [navigate, appliedQuery, user])

  useEffect(() => {
    if (!activeVideo) return
    // Seed the like/dislike glow from the active video's own flags, so it is
    // correct no matter how the active video was set (initial load, next/prev,
    // background refresh, or list click).
    setLiked(Boolean(activeVideo.likedByUser))
    setDisliked(Boolean(activeVideo.dislikedByUser))
    addWatchedVideo({
      id: activeVideo.id,
    ...
    // Keyed on the video's identity (id), not the object reference — so updating
    // counts/flags on the same video doesn't re-run this and clobber the glow.
  }, [activeVideo?.id, user])
  ...
  const applyVideoCounts = (
    ...
    setVideos((current) => current.map((video) => (video.id === videoId ? patch(video) : video)))
    setActiveVideo((current) => (current && current.id === videoId ? patch(current) : current))
  }
  ...
  const handleVideoSelect = (video: Video) => {
    ...
    setInteractionError('')
    setLiked(Boolean(video.likedByUser))
    setDisliked(Boolean(video.dislikedByUser))
  }
  ...
  const toggleLike = async () => {
    ...
    try {
      ...
      if (typeof res.data?.likes === 'number' && typeof res.data?.dislikes === 'number') {
        ...
        likes = Math.max(0, activeLikeCount + likeDelta)
        dislikes = Math.max(0, activeDislikeCount + dislikeDelta)
      }
      applyVideoCounts(activeVideo.id, likes, dislikes, nextLiked, nextDisliked)
  ...
  const toggleCommentLike = async (commentId: number, e?: React.MouseEvent) => {
    e?.preventDefault()
    if (!user) {
  ...
  const toggleCommentDislike = async (commentId: number, e?: React.MouseEvent) => {
    e?.preventDefault()
    if (!user) {
                      ...
                      <div className="rounded-3xl border border-border bg-background/80 p-5 shadow-sm">
                        ...
                        <div className="mt-4 flex flex-wrap gap-3">
                          ...
                          >
                            ...
                            {liked ? 'Liked' : 'Like'} · {activeLikeCount}
                          </button>
                          <button
                          ...
                          >
                            ...
                            {disliked ? 'Disliked' : 'Dislike'} · {activeDislikeCount}
                          </button>
                          <button
                                ...
                                <div className="mt-3 flex flex-wrap items-center gap-3 text-sm">
                                  <button
                                    ...
                                    className={`inline-flex items-center gap-2 rounded-full border border-border px-3 py-2 text-xs ${
                                      comment.likedByUser ? 'bg-green-500/15 text-green-300' : 'bg-background/90 text-foreground hover:bg-secondary'
                                    }`}
                                  >
                                  ...
                                  <button
                                    type="button"
                                    onClick={(e) => toggleCommentDislike(comment.id, e)}
                                    className={`inline-flex items-center gap-2 rounded-full border border-border px-3 py-2 text-xs ${
                                      comment.dislikedByUser ? 'bg-red-500/15 text-red-300' : 'bg-background/90 text-foreground hover:bg-secondary'
                                    }`}
                                  >
